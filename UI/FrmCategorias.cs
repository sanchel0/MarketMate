...
namespace UI
{
    ...
    public partial class FrmCategorias : BaseFormObserver
    {
        ...
        private void AplicarEliminar()
        {
            ...
            {
                throw new ValidationException(ValidationErrorType.NoGridSelection);
            }
        }
        ...
        private void UpdateGrid()
        {
            ...
            /*_categoriasParaMostrar = new List<CategoriaBE>();

            foreach (var categoria in _categorias)
            {
                CategoriaBE copiaCategoria = new CategoriaBE(
                    categoria.Nombre,
                    categoria.Descripcion
                );

                _categoriasParaMostrar.Add(copiaCategoria);
            }*/

            /*if(SessionManager.Language == Language.en)
                _categoriasParaMostrar = _categorias.Select(c => TranslateToEnglish(c)).ToList();*/

            ControlHelper.UpdateGrid(dgvCategorias, _categoriasParaMostrar, "Codigo");
            //Primero se carga el dgv con los datos en español, luego si el idioma es english, TranslateGrid traduce esa grilla a ingles.
            TranslateGrid(dgvCategorias);
